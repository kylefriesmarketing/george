...
horse(r){ let s=wash(r,'#9db0bd','#93a7b6');
  ...
  /* the horse */
  s+=`<path d="M120 96 L200 96 L192 132 L128 132 Z" fill="#8a7148"/>`
    +R(120,92,80,7,'#9c8258')+L(120,99,200,99,'#6b5636',1.6,.8)
    +L(132,132,128,150,'#6b5636',3)+L(188,132,192,150,'#6b5636',3);
  /* a vaulter mid-air, hands on the horse */
  s+=E(160,74,3,3.4,P.ink,.95)+R(156.4,78,7.2,10,P.ink,.95)
    +L(158,88,150,96,P.ink,2.4,.95)+L(163,88,172,94,P.ink,2.4,.95);
  /* the queue */
  s+=E(84,120,2.6,3,'#2c3036',.9)+R(81,123,6,11,'#2c3036',.9);
  s+=E(70,122,2.6,3,'#2c3036',.85)+R(67,125,6,11,'#2c3036',.85);
  s+=E(250,118,2.8,3.2,'#37424d',.9)+R(246.6,121,6.8,12,'#37424d',.9); /* a bored goon */
  s+=E(160,152,40,6,P.soild,.5);
  return s+grain(r); },

inhorse(r){ let s=R(0,0,320,180,'#151009');
  /* plywood walls converging */
  s+=`<path d="M0 0 L320 0 L260 40 L60 40 Z" fill="#2e2312"/>`
    +`<path d="M0 180 L320 180 L260 140 L60 140 Z" fill="#241a0c"/>`
    +`<path d="M0 0 L60 40 L60 140 L0 180 Z" fill="#291e0e"/>`
    +`<path d="M320 0 L260 40 L260 140 L320 180 Z" fill="#291e0e"/>`;
  for(let i=0;i<5;i++){ s+=L(60+i*40,40,60+i*40,140,'#3a2c15',1.2,.5); }
  /* the digger folded, the lamp, the bowl */
  s+=E(150,108,26,16,'#4a3a20',.9)+E(136,96,7,8,'#5c4826',.95);
  s+=E(178,120,8,5,'#6b5636',.9); /* the bowl */
  s+=E(196,100,6,7,'#d8a24a',.5)+E(196,100,3,4,'#f0c060',.7); /* margarine lamp */
  s+=E(160,90,60,40,'#d8a24a',.06);
  /* the open trap below */
  s+=R(120,128,60,14,'#0d0a06')+L(120,128,180,128,P.sandd,1.4,.6);
  /* boots overhead: thud marks */
  for(let i=0;i<4;i++) s+=E(80+i*54,18,10,4,'#0a0805',.8);
  return s; },
